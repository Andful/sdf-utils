...
fn main() -> anyhow::Result<()> {
    ...
    let mut cycle_time_ub = milp
        .hsdf
        .repetition_vector
        .iter()
        .enumerate()
        .map(|(i, r)| r[0] * execution_time.get(&i).unwrap())
        .sum::<usize>();

    let model = &mut milp.model;
    let mut cycle_time_constraint = model.add_constr(
        "no_deadlock",
        grb::c!(throughput.clone() * cycle_time_ub >= 1),
    )?;
    let mut capacity_constraint =
        model.add_constr("capacity", c!(buffer_size.clone() <= grb::INFINITY))?;
    ...
    loop {
        model.set_objective(
            0.0 * throughput.clone() + buffer_size.clone(),
        ...
        let capacity: usize = model
            .get_obj_attr_batch(attr::X, buffers.clone())?
            .iter()
            .map(|e| e.round() as usize)
            .sum();
        capacity_constraint = model.add_constr("capacity", c!(buffer_size.clone() <= capacity))?;
        model.set_objective(
            cycle_time_ub * throughput.clone() + 0.0 * buffer_size.clone(),
            grb::ModelSense::Maximize,
        )?;
        ...
        cycle_time_constraint = model.add_constr(
            "no_deadlock",
            grb::c!(throughput.clone() * cycle_time_ub >= 1),
        )?;
...
mod tests {
    ...
    fn test() -> grb::Result<()> {
        ...
        println!(
            "x:{}\ty:{}",
            model.get_obj_attr(attr::X, &x)?,
            model.get_obj_attr(attr::X, &y)?
        );
