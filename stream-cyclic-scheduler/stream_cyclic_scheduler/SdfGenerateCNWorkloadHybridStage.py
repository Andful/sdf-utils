...
class SdfGenerateCNWorkloadHybridStage(Stage):
    """
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.finer_nodes_dict: dict[ComputationNode,
    ...
    def generate_finer_nodes(self) -> Generator[ComputationNode, None, None]:
        for node in self.workload.nodes():
            ...
            assert isinstance(
                node, ComputationNode), "Only computational nodes are supported"

            yield self.get_finer_node(node, self.get_outer_tmap_loop_dimensions(node))
    ...
    def get_all_node_pairs(G: Workload) -> tuple[tuple[ComputationNode, ComputationNode, bool], ...]:
        ...
        for node in G.topological_sort():
            ...
            is_computation_node = [isinstance(
                succ, ComputationNode) for succ in successors]
            while not all(is_computation_node):
                non_computation_node_succ_idx = is_computation_node.index(
                    False)
    ...
    def get_outer_tmap_loop_dimensions(self, layer: ComputationNode) -> List[TemporalLoop]:
        ...
        """
        window = self.tile_window.get(layer.name)
        if window == None:
            raise Exception(f"Window unspecified for layer {layer.name}")
        (wx, wy) = window
        if wx == None:
            dim1 = 1
        else:
            dim1 = layer.layer_dim_sizes[LayerDim('OX')]//wx

        if wy == None:
            dim2 = 1
        else:
            dim2 = layer.layer_dim_sizes.data[LayerDim('OY')]//wy

        outer_loops: list[TemporalLoop] = [TemporalLoop(
            LayerDim('OX'), dim1), TemporalLoop(LayerDim('OY'), dim2)]
    ...
    def get_non_type_predecessors(self, node: Node, types: list[type]) -> list[Node]:
        ...
        while any([type(pred) in types for pred in preds]):
            ...
            skip_node = next(pred for pred in preds if any(
                [isinstance(pred, type) for type in types]))
            # Find its index
    ...
    def get_group_id(node: ComputationNode, loop_ranges: LoopRanges, groups: dict) -> tuple[int, dict]:
        ...
        relevant_dims = node.loop_relevancy_info.get_r_layer_dims(
            constant_operand)
    ...
    def get_finer_node(
        ...
        for outer_tl in outer_temporal_loops:
            ...
            # We can allow remeinder not equal to 0
            assert (
              rem == 0
        ...
        cn = ComputationNode(
            ...
            output_names=original_node.output_names,
            op_type=original_node.type,
            sub_id=0
        )
        cn.number_execution = number_execution
        return cn
    ...
    def get_finer_nodes(
        ...
        stop_values = [
            temporal_loop.size for temporal_loop in outer_temporal_loops]
        ...
        for n in range(nb_cns):
            ...
            for loop_dim in loop_dims:
                ...
                dim_max = dim_min + \
                    (finer_span[loop_dim] if loop_dim in finer_span else 1)
            ...
            group_id, groups = MyGenerateCNWorkloadHybridStage.get_group_id(
            ...
            produces_final_output = all(
                [dim_min_max[dim][1] >= original_node.layer_dim_sizes[dim]
            ...
            for constant_operand in finer_node.constant_operands:
                ...
                tensor.set_base_priorities(
                    tensor_reuse_factors[constant_operand][n])
    ...
    def get_intra_edges(nodes: list[ComputationNode]):
        ...
        intra_edges: list[tuple[ComputationNode,
                                ComputationNode, dict[str, int]]] = []
    ...
    def get_bounding_box_dimensions(
        ...
        dimensions, loop_ranges = self.flatten_grouped_convolution_ranges(
            producer, consumer, dimensions, loop_ranges)
    ...
    def bounding_box_generator(
        ...
        for i, node in enumerate(nodes):
            inclusive_ranges = self.convert_to_inclusive_data_range(
                node.loop_ranges)
            dimensions = node.operand_dimensionality_order[operand]
            bounds = self.get_bounding_box_dimensions(
                producer, consumer, dimensions, inclusive_ranges)
            yield (i, bounds, None)
    ...
    def get_nb_input_dimensions(self, node: ComputationNode):
        ...
        input_operand = list(set(node.input_operands) -
                             set(node.constant_operands))[0]
    ...
    def build_rtree(
        ...
        rtree = index.Index(self.bounding_box_generator(
            producer, consumer, nodes, operand), properties=props)
        return rtree
    ...
    def flatten_grouped_convolution_ranges(
        ...
        if dim_G in dims_copy and (dim_C in dims_copy or dim_K in dims_copy):
            ...
            CH_max_incl = G_max_incl * \
                original_node.layer_dim_sizes[dims_copy[C_K_idx]
                                              ] + C_K_max_incl
    ...
    def get_inter_edges_rtree(
        ...
        edges: list[tuple[ComputationNode,
                          ComputationNode, dict[str, Any]]] = []

        for input_operand in dependent_input_operands:
            # Build the tree of all finer consumer nodes for this operand
            consumer_tree = self.build_rtree(
                producer, consumer, finer_consumers, input_operand)
            ...
            # node's output
            producer_r_dims_output = producer.operand_dimensionality_order[
                Constants.OUTPUT_LAYER_OP]
            producer_ir_dims_output = producer.loop_relevancy_info.get_ir_layer_dims(
                Constants.OUTPUT_LAYER_OP)
            ...
            for finer_producer in finer_producers:
                ...
                p_inclusive_ranges = self.convert_to_inclusive_data_range(
    ...
    def get_inter_edges_numpy(
        ...
        all_inter_edges: list[tuple[ComputationNode,
                                    ComputationNode, dict[str, Any]]] = []
        for path_between in paths_between_generator:
            ...
            node = path_between[0]
            assert isinstance(
                node, ComputationNode), "First node in path should be ComputationNode"
    ...
    def get_inter_edges_tensor_based(
        ...
        inter_edges: set[tuple[np.ndarray[Any, Any],
                               np.ndarray[Any, Any]]] = set()
    ...
    def get_tensor_cns(
        ...
        is_source_node = len(
            self.get_non_type_predecessors(node, [DummyNode])) == 0
        ...
        tensor_shapes = {op: tuple([all_loop_dim_sizes[dim] for dim in dims]) for (
            op, dims) in tensor_dims.items()}
        ...
        for op in variable_operands:
            ...
            if op == node.output_operand:
                ir_dims_output = node.loop_relevancy_info.get_ir_layer_dims(
                    Constants.OUTPUT_LAYER_OP)
                finer_nodes_list = finer_nodes  # list in regular order
                should_add_to_tensor_list = [
                    all([finer_node.loop_ranges[ir_dim][1] >=
                        node.loop_ranges[ir_dim][1] for ir_dim in ir_dims_output])
                ...
                finer_nodes_list = list(reversed(finer_nodes))
            ...
            for finer_node, should_add_to_tensor in zip(finer_nodes_list, should_add_to_tensor_list):
                ...
                op_dim_ranges = [finer_node.loop_ranges[loop_dim]
                                 for loop_dim in dims]
                op_dim_ranges_max_stop = tuple(tensor_shapes[op])
                window = tuple(
                    [slice(max(0, start), stop)
                     for (start, stop) in op_dim_ranges]
                ...
                nb_unique_data_bits = np.sum(
                    tensors_cns[op][window] == set()) * precision
                ...
                for index in itertools.product(*bounded_op_dim_ranges):
                    ...
                    tensors_cns[op][index] |= {finer_node}
            if nb_unique_data_seen != (prod(tensor_shapes[op]) * precision):
                logger.warn(f"Downsampling node detected: {
                            node}, operand= {op}.")
    ...
    def set_base_priority_of_nodes(G: Workload, finer_nodes_dict: dict[ComputationNode, list[ComputationNode]]):
        ...
        """
        nb_nodes_per_layer_id = {layer.id: len(
            finer_nodes_dict[layer]) for layer in finer_nodes_dict.keys()}
        ...
        for node in G.topological_sort():
            ...
            for layer_operand in node.layer_operands:
                ...
                if layer_operand == node.output_operand:
                    ...
                    successors = [succ for succ in G.successors(
                        node) if succ.id != layer_id]
    ...
    def set_nb_real_predecessors(self, G: Workload):
        ...
        for n in G.nodes():
            nb_real_predecessors = len(
                list(pred for pred in G.predecessors(n) if pred.id != n.id))
    ...
    def get_weight_capacities(self):
        ...
        for core in self.accelerator.cores.nodes():
            ...
            core_weight_capacity = core.memory_hierarchy.get_operand_top_level(
                Constants.MEM_OP_2).memory_instance.size
    ...
    def get_layer_split_factors_k(self):
        ...
        for node in self.workload.nodes():
            ...
            if len(core_allocations) == 1:
                continue
            core_capacities = [self.weight_capacities[core_id]
                               for core_id in core_allocations]
...
def deduce_tensor_reuse_factors(
    ...
    total_reuse_factors = {
        op: prod([reuse_factor for (loop_type, reuse_factor)
                 in r_ir_loop[op] if loop_type == "ir"])
