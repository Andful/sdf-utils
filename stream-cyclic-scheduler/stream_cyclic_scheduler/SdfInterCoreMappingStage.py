...
from CyclicFitnessEvaluator import CyclicFitnessEvaluator
...
class SdfInterCoreMappingStage(Stage):
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        for layer_id in map(lambda a: a.id, self.sdf.actors()):
        ...
        self.fitness_evaluator = CyclicFitnessEvaluator(
    ...
    def set_hw_performance_non_flexible_nodes(self):
        ...
        non_flexible_unique_nodes: set[ComputationNode] = set(a for a in self.sdf.actors() if len(self.node_hw_performances[a]) == 1)
        print(f"Non Flexible {non_flexible_unique_nodes}")
        for non_flexible_unique_node in non_flexible_unique_nodes:
            ...
            self.set_hw_performance_node(non_flexible_unique_node, onchip_energy, offchip_energy, latency, core.id)
            non_flexible_unique_node.set_too_large_operands(too_large_operands.copy())
            non_flexible_unique_node.set_offchip_bandwidth(offchip_bw)
