use std::{convert::TryInto, ops::{Add, Mul, Sub, Div, Rem, Index, IndexMut}};
...
impl<const N: usize, E, F> From<F> for Vector<N, E> where [E; N]: From<F> {
    fn from(value: F) -> Self {
        Self(<[E; N]>::from(value))
    }
}

impl<const N: usize, E> Default for Vector<N, E> where E: Default {
    fn default() -> Self {
...
impl<const N: usize, E> Vector<N, E> where E: Ord + Zero + Add<Output=E> + Sub<Output=E> + Rem<Output = E> + Div<Output = E> + Clone,
{
...
impl<const N: usize, E> Vector<N, E> where E: Clone {
    pub fn cast<F>(self) -> Vector<N,F> where F: From<E> {
        let e: [F; N] = match self.0.to_vec().into_iter().map(|e| F::from(e)).collect::<Vec<_>>().try_into() {
            Ok(e) => e,
            Err(_) => unreachable!()
        };
    ...
    pub fn map<F>(self, f: impl FnMut(E) -> F) -> Vector<N,F> {
        let e: [F; N] = match self.0.to_vec().into_iter().map(f).collect::<Vec<_>>().try_into() {
            Ok(e) => e,
            Err(_) => unreachable!()
        };
